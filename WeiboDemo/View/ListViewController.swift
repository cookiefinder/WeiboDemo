...
class ListViewController: UIViewController {
    var weiboContent: [UserWeiboTimeLine.WeiboContent] = []
    ...
    @objc func loadMore() {
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + DispatchTimeInterval.seconds(2)) {
//            self.weiboContent.append("加载更多成功")
            self.tableView.reloadData()
...
extension ListViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: R.reuseIdentifier.weiboListViewCell, for: indexPath)!
        cell.configure(model: weiboContent[indexPath.row])
